 ...
 * <p>
 * IndexController는 서비스 의존성이 없으므로 {@code @MockitoBean} 불필요.
 * <p>
 * 주의: Spring Security 7.x에서 {@code SecurityContextHolderFilter}가 필터 처리 시 SecurityContext를
 * 덮어쓰기 때문에 {@code @WithMockUser} 대신 {@code .with(user(...))} 방식을 사용한다.
 */
class IndexControllerTest extends AbstractControllerTest {
    ...
    void getEmpty_whenAuthenticated_returns200WithModalDiv() throws Exception {
        mockMvc.perform(get("/empty").with(user("testuser")))
               .andExpect(status().isOk())
