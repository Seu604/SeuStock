...
class ShelfControllerTest extends AbstractControllerTest {
    ...
    private static final String SPACE_SHELF_PATH =
            "/spaces/" + SPACE_ID + "/shelves/" + SHELF_ID;
    ...
    void boxList_returnsBoxListFragment() throws Exception {
        mockMvc.perform(get(SPACE_SHELF_PATH + "/boxes").with(user("testuser")))
    ...
    void editModal_returnsEditModalFragment() throws Exception {
        mockMvc.perform(get(SPACE_SHELF_PATH + "/edit").with(user("testuser")))
               .andExpect(status().isOk())
    ...
    void newModal_returnsModalFragment() throws Exception {
        mockMvc.perform(get("/spaces/" + SPACE_ID + "/shelves/new").with(user("testuser")))
               .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("PATCH /spaces/{sid}/shelves/{id} - 정상 name → shelf-list-response 프래그먼트 + HX-Trigger")
    void rename_withValidName_returnsShelfListResponseWithToast() throws Exception {
        mockMvc.perform(patch(SPACE_SHELF_PATH)
                        .with(user("testuser"))
                        .with(csrf())
                        .param("name", "수정된 선반"))
               .andExpect(status().isOk())
    ...
    void create_withValidName_returnsShelfListResponseWithToast() throws Exception {
        ...
        mockMvc.perform(post("/spaces/" + SPACE_ID + "/shelves")
                        .with(user("testuser"))
                        .with(csrf())
                        .param("name", "새 선반"))
               .andExpect(status().isOk())
    ...
    void delete_returnsShelfListWithToast() throws Exception {
        mockMvc.perform(delete(SPACE_SHELF_PATH)
                        .with(user("testuser"))
                        .with(csrf()))
               .andExpect(status().isOk())
