 ...
 * 검증 범위:
 * <ul>
 *   <li>Security: 모든 엔드포인트 미인증 → /login 리다이렉트, POST CSRF 검증</li>
 *   <li>Validation: 빈 이름 제출 시 동일 뷰 반환, HX-Trigger 없음</li>
 *   <li>Response Shape: full-page vs. HTMX 프래그먼트 뷰 이름, append=true 분기</li>
 *   <li>GlobalExceptionHandler: HTMX 요청 시 에러 모달 프래그먼트 반환</li>
 * </ul>
 */
class SpaceControllerTest extends AbstractControllerTest {
    ...
    void stubDefaults() {
        ...
        given(spaceService.findByExternalId(any(), anyString()))
                .willReturn(stubSpace());
        given(shelfService.findAllBySpaceId(any(), anyString()))
                .willReturn(List.of());
        given(stockService.findPanelBySpace(any(), anyString()))
                .willReturn(List.of());
        given(spaceService.update(any(), any(), anyString()))
                .willReturn(stubSpace());
    }
    ...
    void list_authenticated_returns200WithSpacesListView() throws Exception {
        mockMvc.perform(get("/spaces").with(user("testuser")))
    ...
    void list_withAppendTrue_returnsMoreResponseFragment() throws Exception {
        mockMvc.perform(get("/spaces").with(user("testuser")).param("append", "true"))
               .andExpect(status().isOk())
    ...
    void detail_authenticated_returns200WithDetailView() throws Exception {
        mockMvc.perform(get("/spaces/" + SPACE_ID).with(user("testuser")))
               .andExpect(status().isOk())
    ...
    void editRow_returns200WithEditFragment() throws Exception {
        mockMvc.perform(get("/spaces/" + SPACE_ID + "/edit").with(user("testuser")))
               .andExpect(status().isOk())
    ...
    void cancelEdit_returns200WithViewFragment() throws Exception {
        mockMvc.perform(get("/spaces/" + SPACE_ID + "/cancel").with(user("testuser")))
               .andExpect(status().isOk())
    ...
    void updateRow_withValidName_returnsViewFragmentWithToast() throws Exception {
        mockMvc.perform(put("/spaces/" + SPACE_ID)
                        .with(user("testuser"))
                        .with(csrf())
                        .param("name", "수정된 창고"))
               .andExpect(status().isOk())
    ...
    void delete_returnsListSectionWithToast() throws Exception {
        mockMvc.perform(delete("/spaces/" + SPACE_ID)
                        .with(user("testuser"))
                        .with(csrf()))
               .andExpect(status().isOk())
