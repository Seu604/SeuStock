 ...
 * 검증 범위:
 * <ul>
 *   <li>Security: 3개 엔드포인트 미인증 → /login 리다이렉트</li>
 *   <li>Response Shape: GET /images/{id} → 200, Cache-Control, ETag 헤더</li>
 *   <li>Async: POST /images/analyze 파일 업로드 → asyncStarted → 200 JSON</li>
 *   <li>Async: POST /images/{id}/analyze → asyncStarted → 200 JSON</li>
 * </ul>
 * 특이사항: {@code aiAnalysisExecutor}는 실제 빈을 사용하고
 * {@code ImageAnalysisService}를 Mock으로 대체하여 Ollama 호출 없이 테스트한다.
 */
class ImageControllerTest extends AbstractControllerTest {
    ...
    void show_authenticated_returns200WithCacheHeaders() throws Exception {
        mockMvc.perform(get(IMAGE_PATH).with(user("testuser")))
