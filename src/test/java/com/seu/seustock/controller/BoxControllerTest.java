 ...
 * <p>
 * 특이사항: POST /shelves/{shelfId}/boxes 는 {@code spaceExternalId}를
 * {@code @PathVariable}이 아닌 {@code @RequestParam}으로 받는다.
 */
class BoxControllerTest extends AbstractControllerTest {
    ...
    static Stream<Arguments> protectedEndpoints() {
        ...
        return Stream.of(
                ...
                Arguments.of("DELETE", "/spaces/" + SPACE_ID + "/shelves/" + SHELF_ID + "/boxes/" + BOX_ID)
        );
    }
    ...
    void editModal_whenUnauthenticated_redirectsToLogin() throws Exception {
        mockMvc.perform(get(SPACE_SHELF_BOX_PATH + "/edit"))
               .andExpect(status().is3xxRedirection())
               .andExpect(redirectedUrl("/login"));
    }
    ...
    void create_whenUnauthenticated_redirectsToLogin() throws Exception {
        mockMvc.perform(post("/shelves/" + SHELF_ID + "/boxes")
                        .with(csrf())
    ...
    void editModal_returnsEditModalFragment() throws Exception {
        mockMvc.perform(get(SPACE_SHELF_BOX_PATH + "/edit").with(user("testuser")))
    ...
    void newModal_returnsModalFragment() throws Exception {
        mockMvc.perform(get("/spaces/" + SPACE_ID + "/shelves/" + SHELF_ID + "/boxes/new")
                        .with(user("testuser")))
               .andExpect(status().isOk())
    ...
    void rename_withValidName_returnsBoxListResponseWithToast() throws Exception {
        mockMvc.perform(patch(SPACE_SHELF_BOX_PATH)
                        .with(user("testuser"))
                        .with(csrf())
                        .param("name", "수정된 박스"))
               .andExpect(status().isOk())
    ...
    void delete_returnsBoxListContainerWithToast() throws Exception {
        mockMvc.perform(delete(SPACE_SHELF_BOX_PATH)
                        .with(user("testuser"))
                        .with(csrf()))
               .andExpect(status().isOk())
