 ...
 * <p>
 * {@code HtmxExceptionHandlerExceptionResolver}의 {@code @RequestHeader} 파라미터 해결 제한으로
 * {@code @SpringBootTest + MockMvc}로는 HTMX 분기를 테스트할 수 없어 직접 단위 테스트로 검증한다.
 * <p>
 * 검증 범위:
 * <ul>
 *   <li>예외 종류별 올바른 뷰 이름 반환</li>
 *   <li>HX-Request 헤더 유무에 따른 에러 모달 vs. 에러 페이지 분기</li>
 *   <li>모델에 {@code statusCode}, {@code errorTitle} 속성이 설정됨</li>
 * </ul>
 */
class GlobalExceptionHandlerTest {
    ...
    void handleNotFound_withHxRequest_returnsErrorModalFragment() {
        String view = handler.handleNotFound(
                new NoSuchElementException("공간을 찾을 수 없습니다."), "true", model);
    ...
    void handleNotFound_withoutHxRequest_returnsFullErrorPage() {
        String view = handler.handleNotFound(
                new NoSuchElementException("공간을 찾을 수 없습니다."), null, model);
    ...
    void handleNotFound_withHxRequestFalse_returnsFullErrorPage() {
        String view = handler.handleNotFound(
                new NoSuchElementException("공간을 찾을 수 없습니다."), "false", model);
    ...
    void handleForbidden_withHxRequest_returnsErrorModalFragment() {
        String view = handler.handleForbidden(
                new SecurityException("접근 권한이 없습니다."), "true", model);
    ...
    void handleForbidden_withoutHxRequest_returnsFullErrorPage() {
        String view = handler.handleForbidden(
                new SecurityException("접근 권한이 없습니다."), null, model);
    ...
    void handleBadRequest_withIllegalArgument_andHxRequest_returnsErrorModalFragment() {
        String view = handler.handleBadRequest(
                new IllegalArgumentException("잘못된 요청입니다."), "true", model);
