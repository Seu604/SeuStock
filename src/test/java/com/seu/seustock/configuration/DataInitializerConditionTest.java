...
class DataInitializerConditionTest {
    ...
    static class MockDependenciesConfig {
        ...
        @Bean SpaceMapper spaceMapper() { return mock(SpaceMapper.class); }
        @Bean ShelfMapper shelfMapper() { return mock(ShelfMapper.class); }
        @Bean BoxMapper boxMapper() { return mock(BoxMapper.class); }
        @Bean ItemMapper itemMapper() { return mock(ItemMapper.class); }
        @Bean StockMapper stockMapper() { return mock(StockMapper.class); }
        @Bean StockTransactionMapper stockTransactionMapper() { return mock(StockTransactionMapper.class); }
        @Bean ItemImageMapper itemImageMapper() { return mock(ItemImageMapper.class); }
        @Bean StockImageMapper stockImageMapper() { return mock(StockImageMapper.class); }
        @Bean PasswordEncoder passwordEncoder() { return mock(PasswordEncoder.class); }
        @Bean ImageStorageService imageStorageService() { return mock(ImageStorageService.class); }
    }
}
