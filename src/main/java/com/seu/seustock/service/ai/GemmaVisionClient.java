...
public class GemmaVisionClient {
    ...
    public ImageAnalysisDTO analyze(byte[] imageBytes,
                                    String mimeType,
                                    int retryAttempt,
                                    String previousName,
        ...
        log.info("[GemmaVisionClient] Ollama 호출 시작 - retryAttempt={}, yoloDetections={}",
                normalizedRetryAttempt, detections.size());
        ...
        ImageAnalysisDTO result = chatClient.prompt()
                ...
                .user(user -> user
                        .text(userPrompt(normalizedRetryAttempt,
                                previousName,
                                previousDescription,
                                detections,
                                outputConverter))
    ...
    private String yoloSummary(List<YoloDetection> detections) {
        ...
        for (int i = 0; i < limit; i++) {
            ...
            sb.append("- label: ").append(blankToDash(detection.label()))
                    .append(", confidence: ").append(detection.confidence() == null ? "-" : String.format("%.2f", detection.confidence()));
            if (detection.x1() != null && detection.y1() != null && detection.x2() != null && detection.y2() != null) {
                sb.append(", box: [")
                        .append(Math.round(detection.x1())).append(", ")
                        .append(Math.round(detection.y1())).append(", ")
                        .append(Math.round(detection.x2())).append(", ")
                        .append(Math.round(detection.y2())).append("]");
            }
            sb.append('\n');
        }
        return sb.toString();
