...
/**
 * 비밀번호 재설정 토큰을 Redis에 저장/조회한다. 토큰은 TTL이 지나면 자동 만료되며,
 * 사용 즉시 삭제(1회용)한다. 재발송 남용을 막기 위해 이메일 단위의 쿨다운 키도 관리한다.
 *
 * <p>{@code StringRedisTemplate}은 세션 저장소 설정(store-type)과 무관하게
 * {@code spring.data.redis.*}가 설정되어 있으면 자동 구성된다.
 */
@Component
