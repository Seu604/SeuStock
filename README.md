...
SeuStock은 개인 또는 소규모 팀이 공간, 선반, 박스 단위로 물품과 재고를 관리할 수 있도록 만드는 웹 기반 재고 관리 애플리케이션입니다. Spring Boot, Thymeleaf, HTMX 기반 서버 렌더링 UI를 사용하고, MyBatis로 PostgreSQL/H2 데이터베이스에 접근합니다.
...
기본 접속 주소는 `http://localhost:8080`입니다. `local` 프로필은 로컬 PostgreSQL, MinIO, Redis, Ollama, YOLO 서비스 주소를 설정합니다.
...
- 이미지 저장의 기본 구현은 `MinioImageStorageService`입니다. `LocalImageStorageService`는 같은 인터페이스를 구현한 로컬 파일 저장 구현입니다.
