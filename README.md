...
- 기존 Gemma 단독 이미지 분석 흐름을 `service.ai.ImageAnalysisService` 인터페이스 기반으로 분리하고, 구현체에서 리사이징 → YOLO 전처리 → Gemma 최종 추론 순서로 처리할 예정입니다.
...
- `service.ai.GemmaVisionClient`가 이미 `ChatClient` (Spring AI Ollama)를 사용해 프롬프트 구성, 온도 조절, retry 파라미터 처리, `BeanOutputConverter`로 구조화된 응답 파싱까지 수행한다. 동일한 패턴으로 텍스트 기반 AI 평가 서비스를 구현할 수 있다.
...
- `ImageStorageService`와 `service.ai.ImageAnalysisService` 인터페이스는 `MultipartFile`을 받아 처리하는 범용 흐름으로 재사용 가능하다.
...
- `service.ai.ImageAnalysisService`가 인터페이스로 분리되어 컨트롤러는 분석 구현체에 직접 의존하지 않는다.
- `service.ai.YoloGemmaImageAnalysisService`는 서버 최종 리사이징, YOLO 전처리, Gemma 최종 추론을 orchestration한다.
...
| AI 개인 평가 페이지 | ✅ 가능 | 높음 | `service.ai.GemmaVisionClient` ChatClient 패턴 재사용 |
