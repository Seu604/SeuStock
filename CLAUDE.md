...
**Authentication**
- Session-based, not Spring Security. `LoginCheckInterceptor` guards `/spaces/**`, `/stocks/**`, `/shelves/**`, `/boxes/**` by checking `session.getAttribute("loginUser")` (a username string). All controllers read ownership from this attribute.
